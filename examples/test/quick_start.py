...
print(paras)
dc = TsDataCache(data_path=paras['data_path'], sdt=paras['sdt'], edt=paras['edt'])


# 在浏览器中查看单标的单级别的分型、笔识别结果
bars = dc.pro_bar(ts_code='000001.SH', asset='I', start_date=paras['sdt'], end_date=paras['edt'], freq='D')
c = CZSC(bars)
c.open_in_browser()
